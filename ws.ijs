...
NB.* delVar: delete variable on file.
delVar=: 4 : 0
   (endSlash 1!:43'') delVar y     NB. Default to current working directory.
:
   'dirloc fldir'=. getFileDir x
   if. 0 e. $fldir do. 0;'No directory at ','.',~x return. end.
...
NB.* fexist: 1 if named file exists, 0 otherwise.
NB. fexist=: (1:@(1!:4) ::0:@((([:< 8 u:>) ::])&>)@(<^:(L.=0:)))
NB. Use stdlib version instead
...
NB.EG (<1!:43'') unfileVar_WS_&.>'var1';'var2'   NB. Default - left arg unnecessary.
)
